    ...
    <div :class='wrapClass'>
      <ul class='tags'>
        <li class='item' v-for='tag in tags'>
            <a v-bind:href=buildUrl(tag.uuid)>{{tag.name}}</a>
        ...
        <li class='item editable'>
          ...
          <span v-on:click='addTagHander'
            class='tag-editable-addtag'>添加标签
          </span>
        </li>
      </ul>
...
export default {
  props: {
    ...
    link: {
      type: String,
      default: 'http://www.worker.com/tag='
    },
    // 宽度
    width: {
      type: Number,
    ...
    .tags {
      ...
      .item {
        ...
        .tag-input {
          ...
          .transition(border-color 0.2s ease-in-out, box-shadow .4s .2s ease-in-out);
          &:focus{
            border: 1px solid #ccc;
            outline: none;
          }
