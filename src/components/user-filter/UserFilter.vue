<template>
  <div class="dropdown jq-select">
    <button class="btn  btn-sm btn-default dropdown-toggle mr-5"
        @click="dropdownOpenHandler"
        type="button"
        placeholder="{{placeholder}}">{{labelName}}:{{showText}}
        <span class="caret"></span>
    </button>
    <ul class="dropdown-menu check-list-field-container" 
        ...
        <div class="result-view" v-show="!searching">
            ...
            <item v-for="user in selected" v-ref:selected :multi="multi" :user="user"></item>
            <li v-show="selected.length>0" role="separator" class="divider"></li>

            <item v-for="user in defUser" v-ref:defUser :multi="multi" :user="user"></item>
            <!-- <li v-show="defUser.length>0" role="separator" class="divider"></li> -->

            <li v-show="recommandUser.length>0"
              class="dropdown-header">推荐用户:</li>
            <item v-for="user in recommandUser" v-ref:recommandUser :multi="multi" :user="user"></item>

            <li v-show="recommandGroup.length>0"
              class="dropdown-header">推荐用户组:</li>
            <item v-for="user in recommandGroup" v-ref:recommandGroup :multi="multi" :user="user"></item>
        </div>
        <div class="result-view" v-show="searching">
            <li role="separator" class="divider"></li>
            <item v-if="matched.length>0"
              v-for="user in matched"
              :multi="multi" :user="user"></item>
...
export default {
  ...
  methods: {
    ...
    clearSelectHandler (e) {
      ...
      _.each(this.defUser, function (user) {
        user.checked = false
      })
      _.each(this.recommandUser, function (user) {
        user.checked = false
      })
      _.each(this.recommandGroup, function (user) {
        user.checked = false
      })
      // this.selected = []
    },
    ...
    initdata () {
      var data = api()
      // 修饰数据，并且决定是否做一些过滤操作
      var filter = this.multi // 多选模型下才过滤
      data.defUser = this.adornAndFilter(data.defUser, filter)
      data.recommandUser = this.adornAndFilter(data.recommandUser, filter)
      data.recommandGroup = this.adornAndFilter(data.recommandGroup, filter)
